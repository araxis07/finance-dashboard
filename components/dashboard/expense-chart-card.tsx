      ...
      <CardHeader className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            Spending distribution across your recorded expenses.
          </p>
        </div>
        <p className="text-sm font-medium text-muted">
          Total spent: <span className="text-ink">{formatCurrency(totalExpense)}</span>
        </p>
      </CardHeader>
      <CardContent>
        {data.length === 0 ? (
          <EmptyState
            ...
            description="Add an expense transaction to populate the chart and see where your money is going."
            action={<Button onClick={openAddTransaction}>Add expense</Button>}
          />
        ) : (
